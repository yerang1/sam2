...
def load_video_and_init(video_file):
    """업로드된 비디오를 프레임으로 분리하고 첫 프레임과 세션 ID를 반환"""
    ...
    SESSION_STATES[session_id] = {
        ...
        "base": base,              # 첫 프레임 RGB 배열
        "mask_pos": np.zeros(base.shape[:2], dtype=bool),  # 초록색 영역
        "mask_neg": np.zeros(base.shape[:2], dtype=bool),  # 빨간색 영역
        "obj_id_counter": 1,       # 객체 ID 카운터 (Positive, Negative 각각 신규 객체)
        "pending_points": []       # (좌표, label) 쌍 목록
    }
    return Image.fromarray(base), session_id

# ── 클릭 저장 ─────────────────────────────────────────────────
def store_click(evt: gr.SelectData, label, session_id):
    """클릭 위치와 레이블(Positive/Negative)을 pending 목록에 저장"""
...
def apply_pending_clicks(session_id):
    ...
    # 클릭들은 동일한 라벨(모드)로 입력된다고 가정
    mode_label = points[0][1]  # 1: Positive, 0: Negative

    # SAM2는 최소 1개의 positive 포인트가 있어야 마스크 예측 가능하므로
    # Negative 모드에서도 내부적으로는 positive(1)로 변환해 예측 후 red overlay로만 사용
